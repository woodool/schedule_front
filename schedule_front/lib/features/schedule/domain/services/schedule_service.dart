...
class ScheduleService {
  ...
  Future<List<Schedule>> getSchedulesInRange(DateTime start, DateTime end) async {
    final token = await _getIdToken();
    final startDate = start.toIso8601String().split('T').first;
    final endDate = end.toIso8601String().split('T').first;
    
    // GET 메서드 사용 (POST 대신)
    final uri = Uri.parse('${ApiConfig.schedulesEndpoint}/range?startDate=$startDate&endDate=$endDate');
  ...
  Future<void> createSchedule(Schedule schedule) async {
    ...
    if (isRecurringEvent) {
      ...
      body['startTime'] = body['recurrenceStartDate'] ?? schedule.startTime.toIso8601String();
      ...
      if (schedule.recurrenceStartDate == null) {
        ...
        body['recurrenceStartDate'] = schedule.recurrenceStartDate!.toIso8601String();
      }
    ...
    print('  반복 종료일: ${body['recurrenceEndDate'] ?? schedule.recurrenceEndDate?.toIso8601String()}');
    ...
    if (schedule.recurrenceDays != null && schedule.recurrenceDays!.isNotEmpty) {
      ...
      if (days.length == 7) {
        ...
        print('  선택된 요일: ${selectedDays.isNotEmpty ? selectedDays.substring(0, selectedDays.length - 2) : "없음"}');
  ...
  Future<void> updateSchedule(
    ...
    bool preserveExcludedDates = true, // RecurrenceConstants.PRESERVE_EXCLUDED_DATES 기본값 적용
  }) async {
    ...
    print('  옵션: $option' + (occurrenceDate != null ? ', 발생일자: $occurrenceDate' : ''));
    print('  요청 URL: ${ApiConfig.schedulesEndpoint}/${schedule.scheduleId}?$qs');
    print('  excludedDates 보존 여부: $preserveExcludedDates');
    
    final uri = Uri.parse('${ApiConfig.schedulesEndpoint}/${schedule.scheduleId}?$qs');
  ...
  Future<void> _updateExcludeDates(Schedule schedule) async {
    ...
    // API 엔드포인트 구성
    final uri = Uri.parse('${ApiConfig.schedulesEndpoint}/${schedule.scheduleId}?option=ALL');
    ...
    if (response.statusCode != 200) {
      throw Exception('제외 날짜 업데이트 실패: ${response.statusCode} - ${response.body}');
    }
  ...
  Future<void> deleteSchedule(
    ...
    if (isRecurringEvent && option == 'SINGLE' && occurrenceDate != null) {
      ...
      final int excludedCount = excludedDates?.split(',').where((date) => date.isNotEmpty).length ?? 0;
      
      // 이 일정의 recurrenceStartDate와 recurrenceEndDate 사이의 일 수 계산
      final int totalDays = schedule.recurrenceEndDate != null && schedule.recurrenceStartDate != null
          ? schedule.recurrenceEndDate!.difference(schedule.recurrenceStartDate!).inDays + 1
          : 0;
      ...
      print('반복 일정 삭제 체크: 총 발생 $occurrenceCount회, 제외된 날짜 $excludedCount개, 마지막 일정: $isLastRecurrence');
    }
    
    // 시작일 삭제 여부 확인
    bool isDeletingStartDate = false;
    if (isRecurringEvent && occurrenceDate != null && schedule.recurrenceStartDate != null) {
      // 날짜 비교 (시간 무시)
      final occurrenceDay = DateTime(occurrenceDate.year, occurrenceDate.month, occurrenceDate.day);
      final startDay = DateTime(schedule.recurrenceStartDate!.year, 
    ...
    if (isDeletingStartDate && option == 'SINGLE' && !isLastRecurrence) {
      ...
      try {
        ...
        if (schedule.excludedDates != null && schedule.excludedDates!.isNotEmpty) {
          excludedDatesList = schedule.excludedDates!.split(',')
    ...
    final uri = Uri.parse('${ApiConfig.schedulesEndpoint}/$scheduleId?$queryString');
    print('삭제 요청 URI: $uri');
  ...
  Future<Schedule> postponeSchedule(
    ...
    if (isRecurringEvent) {
      ...
      if (originalSchedule.recurrenceStartDate != null && occurrenceDate != null) {
        // 날짜 비교 (시간 무시)
        final occurrenceDay = DateTime(occurrenceDate.year, occurrenceDate.month, occurrenceDate.day);
        ...
        if (isPostponingStartDate) {
          ...
          final newSchedule = Schedule(
            ...
            endTime: DateTime(
              ...
              targetDate.month,
              targetDate.day,
              occurrenceDate.hour + (originalSchedule.endTime.hour - originalSchedule.startTime.hour),
              occurrenceDate.minute + (originalSchedule.endTime.minute - originalSchedule.startTime.minute),
            ),
    ...
    final uri = Uri.parse('${ApiConfig.schedulesEndpoint}/$scheduleId/postpone');
    final response = await http.put(
      uri,
      headers: {
        'Content-Type': 'application/json',
  ...
  Future<Map<String, dynamic>> verifyExcludedDatesPreservation(int scheduleId) async {
    try {
      // 1. 원본 일정 정보 가져오기
      final originalSchedule = await getScheduleById(scheduleId);
      ...
      if (!isRecurringEvent) {
        return {
          'success': false,
          'message': '반복 일정이 아닙니다. 테스트를 진행할 수 없습니다.'
        };
      }
      ...
      if (originalExcludedDates == null || originalExcludedDates.isEmpty) {
        return {
          'success': false,
          'message': '제외된 날짜가 없습니다. 테스트를 진행할 수 없습니다.'
        };
      }
      ...
      return {
        ...
        'recurrence_start_date': originalSchedule.recurrenceStartDate?.toIso8601String(),
        'recurrence_end_date': originalSchedule.recurrenceEndDate?.toIso8601String(),
        'is_recurring': isRecurringEvent,
      };
    } catch (e) {
      return {
        'success': false,
        'message': '테스트 중 오류 발생: $e'
      };
    }
  }
  
  /// 모임일정을 조회합니다. (일정잡기로 생성된 일정 + 모임으로 생성된 일정)
  Future<List<Schedule>> getGroupSchedules() async {
    try {
      final token = await _getIdToken();
      
      // 모임일정 엔드포인트로 요청
      final uri = Uri.parse('${ApiConfig.baseUrl}/planning/schedules');
      final response = await http.get(
        uri,
        headers: { 'Authorization': 'Bearer $token' },
      ).timeout(
        const Duration(seconds: 5),
        onTimeout: () => throw TimeoutException('모임일정 요청 시간 초과'),
      );
      ...
      if (response.statusCode == 200) {
        ...
        print('모임일정 ${schedules.length}개 로드 완료');
        return schedules;
      }
      
      // 백업: 개인 일정 중 모임 관련 일정만 필터링
      print('모임일정 API 호출 실패, 개인 일정에서 모임 일정 필터링: ${response.statusCode}');
      return _getGroupSchedulesFromPersonal();
    } catch (e) {
      // 오류 발생 시 개인 일정에서 모임 관련 일정만 필터링하여 반환
      print('모임일정 로드 실패, 개인 일정에서 모임 일정 필터링: $e');
      return _getGroupSchedulesFromPersonal();
    }
  }
  
  /// 개인 일정 중에서 모임 관련 일정만 필터링하여 반환하는 보조 메서드
  Future<List<Schedule>> _getGroupSchedulesFromPersonal() async {
    try {
      // 모든 개인 일정 가져오기
      final allSchedules = await getSchedules();
      
      return allSchedules.where((schedule) {
        // 1. 일정 유형이 MEETING인 경우
